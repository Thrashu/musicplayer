...
<script>
    export default {
      ...
      mounted(){
          const that =this;
          let url = that.HOST+ "/v1/restserver/ting?method=baidu.ting.billboard.billList&type=1&size=6&offset=0";
          this.$axios.get(url)
            .then(res=>{
              that.todayRecommend=res.data.song_list;
              console.log(that.todayRecommend)
            })
            .catch(err=>{
              console.log(err)
            })
